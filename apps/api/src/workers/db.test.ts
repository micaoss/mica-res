...
describe("Durable Object database adapter — the code paths that broke", () => {
  test("a conditional update inside a transaction reads its row count and continues", async () => {
    const db = await createWorkersDb(fakeDurableStorage());
    await seedUser(db, "a");
    await seedUser(db, "b");

    await db.transaction(async (tx) => {
      const updated = await tx.update(users).set({ status: "disabled" }).where(eq(users.id, "a")).run();
      if (updated.rowsAffected === 0)
        return;
      await tx.delete(users).where(eq(users.id, "b")).run();
    });

    expect((await db.select().from(users).where(eq(users.id, "a")).get())?.status).toBe("disabled");
    expect(await db.select().from(users).where(eq(users.id, "b")).get()).toBeUndefined();
  });
