...
import { registerResourceBindings } from "@/modules/resource";
...
export class AppDurableObject {
  ...
  constructor(state: DurableObjectState, env: WorkerEnv) {
    ...
    void state.blockConcurrencyWhile(async () => {
      ...
      registerResourceBindings(env as Record<string, unknown>);
...
export default {
  async fetch(request: Request, env: WorkerEnv): Promise<Response> {
    // The edge plane answers everything but the admin API without waking the
    // Durable Object: downloads, listings, the registry, S3, the home page.
    const edge = await handleEdge(request, env as never);
    if (edge)
      return edge;
