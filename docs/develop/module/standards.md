...
- An endpoint for other services to call goes on the open API: mount it in `apps/api/src/routes/open.ts`, not in `protected.ts`. That surface has no security headers, CSRF guard, CORS, session or policy middleware — only a per-client rate limit — so a route there authenticates its caller itself. See *Open API* in `docs/architecture.md`.
