...
|---|---|---|---|---|
| `APP_DISPLAY_NAME` | string | `Mica Resources` | no | Human-readable display name. Shown in HTML title, TOTP issuer, etc. |
| `APP_NAME` | string | `mica-res` | no | Application slug — must match /^[a-z][a-z0-9-]*$/. Used as the backup filename prefix, localStorage namespace, and (via VITE_APP_NAME) the frontend build. |
...
| `CF_PURGE_TOKEN` | string | — | no | API token with Cache Purge on that zone. Without it purges are skipped. |
| `CF_ZONE_ID` | string | — | no | Zone id of the download host, for CDN purges. |
...
| `CREATE_RATE_LIMIT_EXEMPT` | string | — | no | Resources to leave unthrottled, comma-separated. Names are the singular collection names derived from the route table — issue, document, comment, attachment, share, group, member, job, tuple, resource-group, totp. |
| `CREATE_RATE_LIMIT_PER_HOUR` | number | `600` | no |  |
| `CREATE_RATE_LIMIT_PER_MINUTE` | number | `60` | no | Per-user caps on content creation. The limiter finds the creating routes itself: a POST whose path also answers GET is creating a member of that collection, so a new module is covered as soon as it mounts. Counted per resource — issue, document, comment, attachment, ... — so a burst of comments does not spend the budget for opening issues. The minute window bounds a burst; the hour window bounds the sustained rate a burst limit alone would still allow. Rejected requests keep counting, so sustained abuse escalates out of the minute window into the hour one. The counters live in the rate_limits table rather than in memory, so they survive a restart — and an eviction on runtimes that evict the app when idle. Either at 0 disables that window; both at 0 disables the gate. |
| `DATA_DIR` | string | — | no | Top-level directory for persistent data (DB, uploads, logs). Anchors the relative paths below. Resolution order mirrors the lode SDK's dataDir() (DATA_DIR > LODE_DIR > ROOT_DIR), but the lode/root dirs are used as a base with a `data/` subdir (e.g. `${LODE_DIR}/data`); DATA_DIR, when set, is the data dir itself. An absolute DB_PATH / LOG_FILE / FILE_STORAGE_LOCAL_ROOT still overrides individually. |
...
| `DB_PATH` | string | `data/db/app.db` | no | Database (relative to DATA_DIR, or absolute) |
| `DEFAULT_ADMIN` | string | — | no | Comma-separated identities that are always admins. A matching login is granted admin on its first login and on every later one, whether or not other admins exist. Entries containing "@" match only an email the IdP marks verified; other entries match the username. The setting grants and never revokes, so demoting a listed admin in the UI lasts only until their next login — remove them from this list instead. |
| `ENABLE_EXPERIMENTAL_DEK_ROTATION` | enum(true,false) | `false` | no | Opt-in flag for the experimental DEK-rotation flow. When false (default) the rotation endpoints respond with 501 Not Implemented. |
...
| `RES_DELETE_GRACE_SECONDS` | number | `604800` | no | Seconds between deleting a resource and removing its bytes (min 60). |
| `RES_DOWNLOAD_URL` | url | `https://dl.res.micaos.dev` | no | Public URL of the R2 custom domain that serves public bytes directly. |
| `RES_HOME_URL` | url | `https://res.micaos.dev` | no | Public URL of the home page, listings and admin API. |
| `RES_KEY_KEK` | string | — | no | 32 random bytes, base64 (`openssl rand -base64 32`). Seals access-key secrets; required to create access keys for protected namespaces. |
| `RES_PROTECT_BUCKET` | string | `protect-res-micaos-dev` | no | Bucket behind the RES_PROTECT binding; it has no public domain. |
| `RES_PUBLIC_BUCKET` | string | `res-micaos-dev` | no | Bucket behind the RES_PUBLIC binding. |
| `RES_S3_URL` | url | `https://s3.res.micaos.dev` | no | Public URL of the S3 read endpoint (path-style). |
| `RES_SIGNED_URL_MAX_TTL_SECONDS` | number | `3600` | no | Longest lifetime in seconds of a signed URL for a protected object. |
| `RES_UPLOAD_TTL_SECONDS` | number | `3600` | no | Lifetime in seconds of a presigned upload URL. |
...
| `SINGLE_USER_MODE` | enum(true,false) | `false` | no | Enable to authenticate against a username + password kept in the env instead of an OIDC provider. Registration and password change are not exposed; rotate by editing these variables and restarting the API. When SINGLE_USER_MODE=true the OAuth /login route is disabled and the SPA renders a local form. |
